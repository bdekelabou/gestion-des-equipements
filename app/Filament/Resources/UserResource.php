...
class UserResource extends Resource
{
    ...
    public static function table(Table $table): Table
    {
        ...
        $table = $table
            ->columns([
                ...
                TextColumn::make('bureau_poste_id')->badge()
                    ->label("Bureau de poste")
                    ->formatStateUsing(fn($state) => BureauPoste::find($state)->nom),
        ...
        if (in_array(LogsAuthentication::class, class_uses_recursive(FilamentAuthentication::getPlugin()->getModel('User')))) {
            $table->pushColumns([
                TextColumn::make('latestSuccessfullAuthentication.login_at')
                    ->dateTime('Y-m-d H:i:s')
        ...
        if (in_array(CanRenewPassword::class, class_uses_recursive(FilamentAuthentication::getPlugin()->getModel('User')))) {
            $table->pushColumns([
                TextColumn::make('latestRenewable.created_at')
                    ->dateTime('Y-m-d H:i:s')
    ...
    {
        return auth()->user()->hasRole(RolesEnums::Inormaticien()->value);
    }
}
